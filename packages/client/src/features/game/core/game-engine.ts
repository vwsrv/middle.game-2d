...
import { TDirection, TPosition } from '@/features/game/models/types';
import { ILevel } from '@/features/game/models/interfaces/level.interface';
import { IGame } from '@/features/game/models/interfaces';

export class AppleWormGameEngine {
  private levels: ILevel[];
  private game: IGame;
  private onUpdate: (game: IGame) => void;
  private animationFrameId: number | null = null;

  constructor(levels: ILevel[], onUpdate: (game: IGame) => void) {
    this.levels = levels;
    this.onUpdate = onUpdate;
    this.game = this.createInitialGameState();
  }

  private createInitialGameState(): IGame {
    return {
      snake: [],
      apples: [],
      score: 0,
  ...
  private moveHead(head: TPosition, direction: TDirection) {
    switch (direction) {
      case 'UP':
        head.y -= 1;
        break;
  ...
  private isReachedBottom(pos: TPosition): boolean {
    return pos.y >= GRID_SIZE - 1;
  }

  private isSegmentSupported(snake: TPosition[]): boolean {
  ...
  private checkCollision(head: TPosition, snake: TPosition[]): boolean {
    const level = this.levels[this.game.currentLevel];
  ...
  private checkExit(head: TPosition, exit: TPosition): boolean {
